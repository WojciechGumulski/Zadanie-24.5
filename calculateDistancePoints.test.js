...
describe('calculateDistancePoints', () => {
  ...
  describe('mamut', () => {
    it('jump more than k-point', () => {
      const actual = calculateDistancePoints(215, 'mamut', 180);
      const expected = 162;
      assert.equal(actual, expected);
    });

    it('jump less than k-point', () => {
      const actual = calculateDistancePoints(182, 'mamut', 190);
      const expected = 110.4;
      assert.equal(actual, expected);
    });

    it('jump exactly than k-point', () => {
      const actual = calculateDistancePoints(185.5, 'mamut', 185.5);
      const expected = 120;
      assert.equal(actual, expected);
    });
  });
